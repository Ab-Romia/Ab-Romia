...
**[commentdraft](https://github.com/Ab-Romia/commentdraft)** &nbsp; `Python · CLI · PyPI · Apache-2.0`

Started as comment handling for a paying client and generalized: a command line tool that triages the comments on a creator's own posts into reply, skip, or escalate, and drafts replies that may state as fact only what one operator-supplied source document states. A question the document does not answer escalates to a person instead of a guess, and nothing posts without a keystroke against that specific reply; there is no `--yes` and no `--all`, and 707 offline tests hold the design in place, including one that drives a real pty to prove pasted keystrokes approve nothing.

The connection research became eight platform guides written from primary sources, 7,036 lines against the tool's 5,612, with every endpoint, scope, and quota carrying the URL it came from and the date it was read. One connector exists, for Facebook Pages, and it has never been run against a live Page. A measured bake-off found the model with the bargain reputation billing 27 times more per comment than the default, where published rates alone predicted a gap near 8; the rest was the prompt cache, serving the default's byte-identical prefix on 28 of 29 calls. [PyPI](https://pypi.org/project/commentdraft/) &nbsp;·&nbsp; [Guides](https://romia.dev/commentdraft) &nbsp;·&nbsp; [Write-up](https://romia.dev/blog/commentdraft-guides-that-did-not-exist).
